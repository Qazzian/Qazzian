 ...
 * name: Perl Style
 * license: LGPL
 * author: Ian Wallis <Ian@Qazzian.co.uk>
 * revision: 1
 * kate-version: 3.4
 * type: indentation
 *
...
function tryCKeywords(line, isBrace)
{
    ...
    var currentString = document.line(currentLine);
    if (currentString.search(/^\s*(if\b|for|foreach|do\b|while|unless|switch|[}]?\s*else|(.*:))/) == -1)
        return -1;
...
function tryCondition(line)
{
    ...
    if (lastChar == ';'
        && currentString.search(/^\s*(if\b|[}]?\s*else|unless|do\b|while\b|for)/) == -1)
    {
        ...
        while (currentLine > 0 && lineDelimiter > 0) {
            ...
            if (firstPosVirtual < indentation) {
                currentString = document.line(currentLine);
                if (currentString.search(/^\s*(if\b|[}]?\s*else|unless|do\b|while\b|for)[^{]*$/) != -1)
...
/**
 * If the non-empty line ends with ); then search for '(' and return its
